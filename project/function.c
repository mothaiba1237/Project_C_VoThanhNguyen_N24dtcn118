...
void addCategory() {
    ...
    do {
        setColor(14);
        printf("Enter category ID (1 - 9999): ");
        setColor(3);
        ...
        if (new_category.id < 1 || new_category.id > 9999) {
            setColor(12);
            printf("Error: Category ID must be between 1 and 9999!\n");
            continue;
        }
    ...
    do {
        ...
        while (new_category.name[0] == ' ') memmove(new_category.name, new_category.name + 1, strlen(new_category.name));
        while (strlen(new_category.name) > 0 && new_category.name[strlen(new_category.name) - 1] == ' ') new_category.name[strlen(new_category.name) - 1] = '\0';

        int length = strlen(new_category.name);
        if (length < 3 || length > 50) {
            setColor(12);
            printf("Error: Name must be 3-50 characters long!\n");
            continue;
        }

        int valid = 1;
        for ( i = 0; i < length; i++) {
            if (!isalnum(new_category.name[i]) && new_category.name[i] != ' ') {
                valid = 0;
                break;
            }
        }
        ...
        if (!valid) {
            setColor(12);
            printf("Error: Name can only contain letters, numbers, and spaces!\n");
            continue;
        }

        is_duplicate = 0;
        for ( i = 0; i < num_categories; i++) {
            if (strcmp(categories[i].name, new_category.name) == 0) {
                setColor(12);
                printf("Error: A category with this name already exists!\n");
                is_duplicate = 1;
                break;
            }
        }
    ...
    for ( i = 0; i < num_categories; i++) {
        if (categories[i].id == id) {
            ...
            do {
                ...
                while (strlen(newName) > 0 && newName[strlen(newName) - 1] == ' ') newName[strlen(newName) - 1] = '\0';
                ...
                for ( j = 0; j < num_categories; j++) {
                    if (j != i && strcmp(categories[j].name, newName) == 0) {
                        setColor(12);
                        printf("Error: A category with this name already exists!\n");
                        is_duplicate = 1;
                        break;
            ...
            strcpy(categories[i].name, newName);
            saveCategoriesToFile();
...
void sortCategoriesByName(int ascending) {
    Category temp;
    for (i = 0; i < num_categories - 1; i++) {
        for (j = i + 1; j < num_categories; j++) {
            if ((ascending && strcmp(categories[i].name, categories[j].name) > 0) ||
                (!ascending && strcmp(categories[i].name, categories[j].name) < 0)) {
                temp = categories[i];
    ...
    setColor(10);
    if (ascending)
        printf("\nCategory list sorted successfully by Name (A - Z)!\n");
    else
        printf("\nCategory list sorted successfully by Name (Z - A)!\n");
...
void showSortCategoryMenu() {
    ...
    do {
        ...
        switch (sortChoice) {
            case 1:
                sortCategoriesByName(1);
                break;
            case 2:
            	sortCategoriesByName(0);
                break;
            case 3:
                return;
            default:
